...
실행명령 : (pyenv) C:\AIProj\StockAnalyzer>python common\crawler\articlecrawler.py
'''
...
class ArticleCrawler(object):
    def __init__(self):
        ...
        self.getday  = {'istoday': False, 'today':''}
    ...
    def set_date(self, isday=False, today=''):
        self.getday['isday'] = isday
        self.getday['today'] = today

    @staticmethod
    def make_news_page_url(category_url, isday, today, start_year, end_year, start_month, end_month):
        made_urls = []
        if isday : 
            # 날짜별로 Page Url 생성
            url = category_url + today
    ...
    def crawling(self, category_name):
        ...
        target_urls = self.make_news_page_url(url_format, self.getday['isday'], self.getday['today'], self.date['start_year'], self.date['end_year'], self.date['start_month'], self.date['end_month'])
        ...
        for url in target_urls:
            ...
            for content_url in post_urls:  # 기사 url
                ...
                try:
                    ...
                    result = {'time': time, 'category_name': category_name, 'text_company': text_company, 'text_headline':text_headline, 'text_sentence': text_sentence, 'content_url': content_url}
                    print(result)
...
if __name__ == "__main__":
    ...
    # Crawler.set_date_range(2021, 4, 2021, 4)
    today =  datetime.today().strftime("%Y%m%d")
    Crawler.set_date(True, today)
    Crawler.start()